...
public class GiftIconListController {
    ...
    public ResponseEntity<Object> addGiftIconList(
        ...
        iGiftIconListService.addGiftIconList(requestHead, requestGiftIconList);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @GetMapping("/get")
    @Operation(summary = "고객이 자신이 가지고 있는 모든 기프티콘 확인", description = "구현 X")
    public ResponseEntity<Object> getGiftIconList(@RequestHeader RequestHead requestHead) {
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(new ResponseDTO(HttpStatus.OK,iGiftIconListService.getGiftIconList(requestHead)));
    }
}
