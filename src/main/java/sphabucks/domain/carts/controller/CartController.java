...
public class CartController {
    ...
    public ResponseEntity<Object> addCart(
            @RequestHeader RequestHead requestHead, @RequestBody RequestCart requestCart){

        // 성공: ResponseEntity.status(HttpStatus.OK).build();
        // 실패: return ResponseEntity.status(HttpStatus.BAD_REQUEST).body((5 - cart.getAmount()));
        return iCartService.addCart(requestHead, requestCart);
    }

    @GetMapping("/get")
    @Operation(summary = "장바구니 조회", description = "uuid 사용")
    public ResponseEntity<Object> getCart(@RequestHeader RequestHead requestHead){
        return ResponseEntity.status(HttpStatus.OK).body(iCartService.getCart(requestHead));
    }

    @GetMapping("/get/v2")
    @Operation(summary = "장바구니 조회 v2", description = "유저의 카트 속 모든 정보를 한번에 반환해줌")
    public ResponseEntity<Object> getCartV2(@RequestHeader RequestHead requestHead) {
        return ResponseEntity.status(HttpStatus.OK).body(iCartService.getCartV2(requestHead));
    }
    ...
    public ResponseEntity<Object> deleteAll(@RequestHeader RequestHead requestHead){
        iCartService.deleteAll(requestHead);
        return ResponseEntity.status(HttpStatus.OK).build();
    }
}